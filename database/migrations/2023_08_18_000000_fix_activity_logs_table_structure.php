...
return new class extends Migration
{
    ...
    private function dropForeignKeysIfExist($table, $columns)
    {
        try {
            ...
            // Get table details
            $foreignKeys = DB::select(DB::raw('SHOW KEYS FROM ' . $table . ' WHERE Key_name LIKE "%_foreign"'));
            
            // Drop each foreign key that references our columns
            foreach ($foreignKeys as $foreignKey) {
                $localColumns = explode(',', $foreignKey->Column_name);
                ...
                if ($shouldDrop) {
                    ...
                    Schema::table($table, function (Blueprint $table) use ($foreignKey) {
                        $table->dropForeign($foreignKey->Key_name);
                    });
                }
            }
        } catch (\Exception $e) {
